...
class Voter(Aggregator):
    ...
    def union_aggregate(self, x, y):
        ...
        for i in range(len(t_list)):
            ...
            f_frame = f_frame.append(f_list[i], sort=True)
...
class Workflow:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.models = eval.train_dag(
            self.dag, train_data=(X, y), sample_weight=sample_weight)
...
class Stacker(Aggregator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for train_idx, test_idx in skf.split(X, y):
            ...
            preproc_X, preproc_y = eval.test_dag(
                self.initial_dag, wf_init.models, test_data=(tr_X, tr_y), output='all')
        ...
        preds = [pd.concat(ps, sort=True) for ps in preds]

        self.train = pd.concat(preds, axis=1, sort=True)
