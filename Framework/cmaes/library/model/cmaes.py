...
class Cmaes:
  ...
  def __setup__(self, x=None, stopeval = None):
    ...
    self.mu_rank_elite_size = max(1, math.floor(self.lambda_pop_size / 2.0))
  ...
  def next(self, fn_eval, counteval, eigenval, fold, optim = 'min'):
    ...
    offspring_evaluation = np.array([fn_eval(mutation_dist[:, offs], fold) for offs in range(self.lambda_pop_size)]).reshape((self.N, 1))
